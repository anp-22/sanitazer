...
=head1 NAME 
    ...
    sanitazer12         Clean files names and title capiatalize names in
                        folders and files from non ascii characters.

=head1 SYNOPSIS 

    --name              Name to sanitaze folder 

=head1 VERSION

our $VERSION = qv('0.5.0');

 Date: Fri Apr 15 01:06:26 PM -04 2022
...
sub movefile {
    ...
    if ( -d $ifile ) {    # are we talking about a directory ?
        ...
        unless ( -d $folder . $ofile ) {    # if does not exits
            ...
            if ( $rc == 0 ) {
                print( $ifile, "  moved to ", $folder . $ofile, "\n" );
            }
            else {
        ...
        while (1) {
            ...
            unless ( -d $folder . $ofile . $vert ) {
                ...
                if ( $rc == 0 ) {
                    ...
                    # print( $ifile, "  moved to ", $folder . $ofile . $vert,
                    # "\n" );
                }
                else {
    ...
    if ( -f $ifile ) {    # are we talking about a regular file ?
        unless ( -f $folder . $ofile . $oext ) {    # if it does not exist already
            ...
            if ( $rc == 0 ) {
                print( $ifile, "  moved to ", $folder . $ofile . $oext, "\n" );
            }
            else {
        ...
        while (1) {
            ...
            unless ( -f $folder . $ofile . $vert . $oext ) {
                ...
                if ( $rc == 0 ) {
                    print( $ifile, "  moved to ", $folder . $ofile . $vert . $oext, "\n" );
                }
                else {
